...
def main():
    ...
    log_path = path.join(log_base_dir, experiment_name)
    ...
    cfg = training_cfg(args.config, "", args.max_iterations, args.num_envs, experiment_name)
    seed = cfg.get("seed", 1)

    # Save snapshot
    pickle.dump(
        {"args": args, "seed": seed, "rsl_rl": cfg},
        open(path.join(log_path, "cfgs.pkl"), "wb"),
    )
    ...
    env = VideoWrapper(
        env,
        video_length_sec=12,
        out_dir=path.join(log_path, "videos"),
